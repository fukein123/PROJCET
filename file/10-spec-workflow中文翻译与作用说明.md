...
> 本仓库当前的 `.spec-workflow` 重点是模板体系与正式规格文档，适合作为“前后端分离项目”的需求与设计标准化入口。
> 当前项目中，审批记录与阶段门禁由 `.approval-workflow + scripts/local-approval.ps1` 承担。
...
- `.spec-workflow/` 继续负责模板与正式规格文档。
- 当前项目的审批工具：
  - 审批脚本：`scripts/local-approval.ps1`
  - 审批记录：`.approval-workflow/`
- 实际批准方式改为聊天文本：
  - 回复 `通过`
  - 或回复 `需修改：<意见>`
